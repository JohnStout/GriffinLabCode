%% SCRIPT_generateInt_laser
%
% this code is meant for generation of the Int file on T-maze tasks with laser onset/offset
% at specific maze locations (startbox/delay, stem, return arm). It provides a method to 
% visualize all trials (and laser location/condition) and tag certain trials with laser error. 
% This code also saves out a Table.
%
% Note: this version of the int file will not include specific reason for trial
% exclusion (ex SCRIPT_generateInt). If that is a consideration for analysis,
% specification of exlcusion criteria will need to be added to
% checkInt_laser script (and the outputs to IntTable)
% Ex. create separate columns in table for exclusion based on tracking/laser at stem, return, delay, etc 
...
prompt = 'Please enter laser location (stem,goal,delay): ';
...
yv_sb = [SB_fld(2) SB_fld(2) SB_fld(2)+SB_fld(4) SB_fld(2)+SB_fld(4) lRW_fld(2)];
...
% startbox entry -- if rat unplugged and SB entrynot registered
try
    Int_old(:,8) = startBox_entry;
catch
    Int_old(:,8) = [startBox_entry, NaN];
    startBox_entry = [startBox_entry, NaN];
end
...
try
    ...
    delayTemp = (stemEntry(2:end)-sbEntry(1:end))./1e6; %for sessions where rat is unplugged and sb entry not registered
end
...
%% CHECK YOUR DATA!!!
[laserID, remTrial] = checkInt_laser(Int_old,x,y,t,vt_name,location);
...
trial2rem = zeros([size(Int_old,1) 1]);
trial2rem(remTrial)=1;
...
% generate table
IntTable = table(trajNumber,stemEntry,cpEntry,gaEntry,gzEntry,retEntry,sbEntry,trajectory,accuracy,trajBinary,accBinary,failedTimeStamp,delayTime,laserID,trial2rem);
